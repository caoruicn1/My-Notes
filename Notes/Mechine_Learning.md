...
*   **背景**
    ...
    *   坑:写代码会发现不同的步长会得出随学习率 alpha 等比例缩放的不同的参数值,
        但其实算的都是同个超平面。因为超平面由下面方程确定： $$w^Tx+b=0$$, 所以
        等比例缩放 w 和 b,超平面不变。
...
*   **基本原理**
    ...
    *   存在一个超平面, 能够刚好把点全部分为正负两类。损失函数为所有点到超平面距
        ...
        断样本是否误判断, 若误判断, 就更新梯度, 最终记录总误判次数。总误判次数会
        收敛。
    *   符号函数:
...
*   **理解函数距离和几何距离**
    ...
    *   点到超平面距离为 $$d=\frac{1}{||w||} |w·x_0+b|$$
    *   所以|w·x+b|可以表示点 x 距离超平面距离的远近,而(w·w+b)·y 表示分类的正确
        性以及确信度。
    *   由此定义函数距离： $$r_i=y_i(w·x_i + b)$$
