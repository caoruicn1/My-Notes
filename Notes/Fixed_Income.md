...
* **U.S. Treasury securities**
  ...
  * Bonds: 30 years --> Coupon-bearing, semi-annual
    * A callable bond: 可被 issuer repurchased.
...
* **The Law of One Price(No Arbitrage)**
  ...
  * $$V= \$425 \times d*{0.5} + \$425 \times d*{1} + \$10425 \times d\_{1.5}$$
  * $$V= K*1 \times d*{t*1} + K_2 \times d*{t*2} + K_n \times d*{t_n}$$
...
* **Implied zero price**
  ...
  1.  已知 97.86 par value of the 1-year bonds, 和 2.10 par ... 0.5-year bond,
      ...
      * 可以运用两个 PV 相减求 1-year zero price:

      $$PV_1 = (97.86 \times \$0.9896875) - (2.10 \times \$0.9940625)$$

      * 若已知条件为 price 和 coupon rate，可以直接列方程组求 discount factor

      $$\begin{cases}
      99.86 = (100+\frac{4.25}{2}) \times d*{0.5}\\
      98.99 = (\frac{4.375}{2}) \times d*{0.5}+(100+\frac{4.375}{2}) \times d\_{0.5}
      \end{cases}$$
...
* **Value of a stream of cash flows**
  * $$V = \sum*{j=1}^{n} K_j \times d_t = \sum*{j=1}^{n} \frac{K*j}{(1 +
    \frac{r*{t_j}}{2})^{2t_j}}$$
  * 注意，每个时期的$$r_t$$都不同
...
* **Definition of yield to maturity**
  ...
  * 之前是分别用每个时期的 discount rate 分别计算每个时期的 cash
    flow，现在重新定义一个能够全部都用来计算的 yield
  ...
  * $$V = \sum\_{j=1}{n} \frac{K_j}{(1+y/2)^{2t_j}}$$
  * Yield is a kind of average of the different zero rates associated with
    the cash flows.

* **Yield of a bond**

  * $$P = \frac{c}{2}\sum\_{s=1}^{2T} \frac{1}{i(1+y/2)} +
    \frac{1}{(1+y/2)^{2T}}$$
  * 利用等比数列公式得
...
* **Value of a stream cash flows**
  ...
  * $$V = \sum*{j=1}^{n} K_j \times d_t = \sum*{j=1}^{n} \frac{K*j}{(1 +
    \frac{r*{t*j}}{2})^{2t_j}} = \sum*{j=1}^{n} \frac{K_j}{(1 + \frac{y}{2})^{2t_j}}$$
...
* **Motivation**
  ...
  * 可以假设成 forward contract 为远期贷款,先把利息交了， spot 为借给别人钱
...
* **Synthetic forward loan**
  ...
  * 思路 1：
    ...
    * 即全部转化为 PV，然后再相除得出 rate, 注意 rate 为年化，且半年付息一次
  * 思路 2:
    * $$ (1+r_t/2)^{2t} (1+f_t^T / 2)^{2(T-t)} = (1+r_T / 2)^{2T}$$
    * 从 0 借到 t，再从 t 借到 T，等于从 0 借到 T
    * ![Picture](what/Fixed_Income_3.png)
  * 思路 3:
    * 已知的为 discount factor
...
* **Ways of reducing the credit exposure**
  * Margin(Haircut)
  * Mark to Market on a regular basis(调整 collateral levels or loan balance)
